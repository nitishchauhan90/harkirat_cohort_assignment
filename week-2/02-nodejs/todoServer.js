 ...
 */
  ...
  //   let todo = todos.filter((id) =>  id == Number(req.params.id));
  ...
  //     todos[id].completed=Boolean(req.body.completed);
  //     res.json();
  //   }
  ...
  app.get('/todos/:id', (req, res) => {
    const todo = todos.find(t => t.id === parseInt(req.params.id));  //statement is used for fast 
    if (!todo) {
      res.status(404).send();
    } else {
      res.json(todo);
